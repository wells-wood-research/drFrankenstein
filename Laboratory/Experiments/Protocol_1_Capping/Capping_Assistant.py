...
def find_bonded_atoms(molDf: pd.DataFrame, atomName: str, distanceCutoff: float=1.6) -> list:
    """
    ...
    tmpDf['distance_to_atom'] = tmpDf.apply(calculate_distance, axis=1, atomX=atomX, atomY=atomY, atomZ=atomZ)
...
def calculate_distance(row: pd.Series, atomX: float, atomY: float, atomZ: float):
    """
    ...
    """
    return np.sqrt((row['X'] - atomX) ** 2 + (row['Y'] - atomY) ** 2 + (row['Z'] - atomZ) ** 2)
...
def apply_transformation(pdbDf: pd.DataFrame, rotationMatrix: np.ndarray, translationVector: np.ndarray) -> pd.DataFrame:
    """
...
def calculate_transformation(originalDf: pd.DataFrame, targetDf: pd.DataFrame, alignmentAtoms: list) -> Tuple[np.ndarray, np.ndarray]:
    """
...
def transform_whole(originalDf: pd.DataFrame, targetDf: pd.DataFrame, atomNames: list) -> pd.DataFrame:
    """
...
def place_capping_atom_by_vector(capDf: pd.DataFrame, vectorAtomCoordsA: np.ndarray, vectorAtomCoordsB: np.ndarray, bondedAtomCoords: np.ndarray, atomNameToPlace: str, bondLength: float) -> pd.DataFrame:
    """
...
def place_capping_atom_by_average_of_vectors(capDf: pd.DataFrame, vectorAtomCoordsA: np.ndarray, vectorAtomCoordsB: np.ndarray, vectorAtomCoordsC: np.ndarray, vectorAtomCoordsD: np.ndarray, bondedAtomCoords: np.ndarray, atomNameToPlace: str, bondLength: float) -> pd.DataFrame:
    """
    Places a capping atom in its DataFrame by:
    1. Calculate vector from atom A to B
    ...
    averageDirection /= np.linalg.norm(averageDirection)
