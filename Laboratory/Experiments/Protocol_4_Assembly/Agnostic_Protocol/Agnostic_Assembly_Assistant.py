...
def find_non_bonded_lookup() -> FilePath:
    ...
    labDir = p.dirname(p.dirname(p.dirname(thisDir)))
    gaffNonBondedLookup = p.join(labDir, "Ingredients", "gaff2_NonBonded_Lookup.yaml")
    absentNonBondedLookup = p.join(labDir, "Ingredients", "Absent_NonBonded_Lookup.yaml")
    return gaffNonBondedLookup, absentNonBondedLookup
