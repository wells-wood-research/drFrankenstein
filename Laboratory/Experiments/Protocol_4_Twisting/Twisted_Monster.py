...
def run_optimisation_step(conformerXyz, conformerScanDir, conformerId, config):
    ...
    optOrcaInput: FilePath = drOrca.make_orca_input_for_opt(inputXyz=conformerXyz, outDir=optDir, moleculeInfo=config['moleculeInfo'], qmMethod=config['torsionScanInfo']['scanMethod'], solvationMethod=config['torsionScanInfo']['scanSolvationMethod'])
...
def run_forwards_scan_step(optXyz, initialTorsionAngle, torsionIndexes, conformerScanDir, conformerId, config):
    ...
    forwardsOrcaInput: FilePath = drOrca.make_orca_input_for_scan(inputXyz=optXyz, outDir=forwardsDir, moleculeInfo=config['moleculeInfo'], qmMethod=config['torsionScanInfo']['scanMethod'], solvationMethod=config['torsionScanInfo']['scanSolvationMethod'], torsionIndexes=torsionIndexes, scanAngles=forwardsScanText)
...
def run_backwards_scan_step(forwardsScanXyz, initialTorsionAngle, torsionIndexes, conformerScanDir, conformerId, config):
    ...
    backwardsOrcaInput: FilePath = drOrca.make_orca_input_for_scan(inputXyz=forwardsScanXyz, outDir=backwardsDir, moleculeInfo=config['moleculeInfo'], qmMethod=config['torsionScanInfo']['scanMethod'], solvationMethod=config['torsionScanInfo']['scanSolvationMethod'], torsionIndexes=torsionIndexes, scanAngles=backwardsScanText)
...
def run_singlepoints_on_scans(scanDir, scanDf, conformerId, config):
    ...
    for scanXyz in scanXyzs:
        ...
        spOrcaInput: FilePath = drOrca.make_orca_input_for_singlepoint(inputXyz=scanXyz, outDir=spDir, moleculeInfo=config['moleculeInfo'], qmMethod=config['torsionScanInfo']['singlePointMethod'], solvationMethod=config['torsionScanInfo']['singlePointSolvationMethod'])
    ...
    singlePointEnergyDf = singlePointEnergyDf.merge(scanDf[['scan_index', 'Angle']], on='scan_index', how='left')
