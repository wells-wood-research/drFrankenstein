...
def how_many_waters_for_solvator(conformerXyzs: list[FilePath], conformerPdb: FilePath, outDir: DirectoryPath, nWatersPerNmSquared: int=25) -> int:
    """
...
def generate_conformer_list_file(orcaCalculationsDir: DirectoryPath, chargeFittingDir: DirectoryPath) -> FilePath:
    """
    ...
    for conformerName in os.listdir(orcaCalculationsDir):
        ...
        if not p.isdir:
            continue
        singlePointOutFile = [p.join(calculationDir, file) for file in os.listdir(calculationDir) if p.splitext(file)[1] == '.out'][0]
...
def calculate_boltzmann_probabilities(df, temperature=298.15):
    ...
    boltzmannFactors = np.exp(-deltaEnergies / (boltzmannConstant * temperature))
...
def apply_resp2_weighted_average(solvatedDf: pd.DataFrame, gasPhaseDf: pd.DataFrame, proportions: list[float]=[0.6, 0.4]) -> pd.DataFrame:
    """
...
def set_up_directories(config: dict, protocol: str) -> dict:
    ...
    if protocol == 'RESP':
        ...
        config['runtimeInfo']['madeByCharges'].update({'chargeDir': chargeDir, 'solvatedDir': solvatedDir, 'gasPhaseDir': gasPhaseDir})
        return config
        ...
        config['runtimeInfo']['madeByCharges'].update({'chargeDir': chargeDir, 'solvatorDir': solvatorDir, 'qmmmOptDir': qmmmOptDir, 'qmmmSinglepointDir': qmmmSinglepointDir, 'fittingDir': fittingDir})
        return config