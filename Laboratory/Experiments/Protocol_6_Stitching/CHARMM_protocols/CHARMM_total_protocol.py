...
def get_mm_total_energies(config: dict, torsionTag: str, debug=True) -> np.ndarray:
    """
...
def get_singlepoint_energies_for_torsion_scan(scanIndex, scanDir, torsionTotalDir, config, debug=False):
    ...
    trajXyzs = sorted([p.join(scanDir, file) for file in os.listdir(scanDir) if re.match('^orca_scan\\.\\d\\d\\d\\.xyz$', file)])
...
def run_mm_singlepoints(trajPdbs: list, config: dict) -> list[float]:
    ...
    system: openmm.System = psf.createSystem(charmmParams, nonbondedMethod=app.NoCutoff, nonbondedCutoff=1 * unit.nanometer, constraints=None)
