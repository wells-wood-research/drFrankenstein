...
def pdb2mol2(inPdb: FilePath, outMol2: FilePath, workingDir: DirectoryPath) -> None:
    """
    ...
    antechamberCommand: list = ['antechamber', '-i', tmpPdb, '-fi', 'pdb', '-o', outMol2, '-fo', 'mol2', '-at', 'gaff2', '-rn', 'MOL', '-s', '2']
    with open(antechamberOut, 'w') as outfile:
        run(antechamberCommand, stdout=outfile, stderr=STDOUT)

def edit_mo2_atom_types(inMol2: FilePath, outMol2: FilePath) -> None:
    """
    ...
    with open(inMol2, 'r') as inMol2, open(outMol2, 'w') as outMol2:
        ...
        for line in mol2Lines:
            if line.strip() == '@<TRIPOS>ATOM':
                ...
                atomName = atomData[1]
                if atomName == 'CA':
                    line = re.sub('(\\s)c3(\\s)', '\\1CX\\2', line)
                line = re.sub('(\\s)os(\\s)', '\\1o \\2', line)
                line = re.sub('(\\s)n7(\\s)', '\\1ns\\2', line)
                line = re.sub('(\\s)h1(\\s)', '\\1hc\\2', line)
            outMol2.write(line)
...
def merge_energy_dfs(energyDfs: list) -> pd.DataFrame:
    ...
    mergedDf = energyDfs[0][['Angle', 'Energy']].rename(columns={'Energy': 'Energy_0'})
    for (i, df) in enumerate(energyDfs[1:], start=1):
        mergedDf = mergedDf.merge(df[['Angle', 'Energy']], on='Angle', how='outer', suffixes=('', f'_df{i}')).rename(columns={'Energy': f'Energy_{i}'})
