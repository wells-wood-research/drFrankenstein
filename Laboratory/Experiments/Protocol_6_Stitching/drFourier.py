...
def construct_cosine_components_charmm(charmmParamDf: pd.DataFrame, angle: np.array, maxFunctions: int, tolerance: float=0.2) -> Tuple[np.array, List[Tuple[float, np.array]], int]:
...
def construct_cosine_components_amber(amberParamDf: pd.DataFrame, angle: np.array, maxFunctions: int, tolerance: float=0.05) -> Tuple[np.array, List[Tuple[float, np.array]], int]:
