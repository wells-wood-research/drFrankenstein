...
def get_donor_acceptors(config: dict, debug: bool=True) -> dict:
    ...
    if debug:
        ...
        tqdmBarOptions = {'desc': f'{purpleText}Assigning Donor / Acceptor Pairs {resetTextColor}', 'ascii': '-🗲→', 'colour': 'yellow', 'unit': 'conformer', 'dynamic_ncols': True}
    ...
    donorAcceptors = {'DONORS': uniqueDonors, 'ACCEPTORS': uniqueAcceptors}
    config['runtimeInfo']['madeByCreator']['donorAcceptors'] = donorAcceptors
    return config

def detect_hydrogen_bonds_construct_donor_acceptors(solvatedOptXyz: FilePath, solvatedDf: pd.DataFrame, moleculeName: str) -> tuple:
        ...
        if rowAtomA['element'] in donorAcceptorTypes:
            for (indexAtomB, rowAtomB) in xyzDf.iterrows():
                ...
                if rowAtomB['element'] == 'H':
                    ...
                    if 0.8 < protonBondLength < 1.2:
                        for (indexAtomC, rowAtomC) in xyzDf.iterrows():
                            coordsC = rowAtomC[['x', 'y', 'z']].values
                            if rowAtomC['element'] in donorAcceptorTypes and indexAtomC != indexAtomA:
                                hydrogenBondLength = calculate_distance(coordsB, coordsC)
                                if hydrogenBondLength < hydrogenBondDistanceCutoff:
                                    angleDha = calculate_angle(coordsA, coordsB, coordsC)
                                    if angleDha > hydrogenBondAngleCutoff:
...
def pad_pdb_with_waters(config: dict) -> FilePath:
    ...
    dummyWaterDf = pd.DataFrame([{'ATOM': 'HETATM', 'ATOM_ID': i, 'ATOM_NAME': name, 'RES_NAME': 'HOH', 'CHAIN_ID': 'A', 'RES_ID': 1, 'X': 0.0, 'Y': 0.0, 'Z': 0.0, 'OCCUPANCY': 1.0, 'BETAFACTOR': 0.0, 'ELEMENT': elem} for (i, (name, elem)) in enumerate([('OH2', 'O'), ('H1', 'H'), ('H2', 'H')], 1)])
    dfsToConcat = [pdbDf]
