...
def make_vibrant_colors():
    vibrantColors = ['#FF0000', '#00FF00', '#0000FF', '#FFFF00', '#FF00FF', '#00FFFF', '#FF4500', '#32CD32', '#1E90FF', '#FFD700', '#FF69B4', '#00CED1', '#FF6347', '#ADFF2F', '#4682B4', '#FFA500', '#DA70D6', '#20B2AA', '#DC143C', '#7CFC00']
...
def draw_task_bars(ax, timeDf, barColors, totalRuntimeSeconds, textColor, backgroundColor, elementColor, defaultFontSize):
    ...
    for (i, row) in timeDf.iterrows():
        ...
        ax.broken_barh(xranges=[(barStartMinutes, barDurationMinutes)], yrange=(yPos - 0.4, 0.8), facecolors=[barColors[i % len(barColors)] if numFunctions > 0 else 'blue'], edgecolor=elementColor, linewidth=1)
        if totalRuntimeSeconds > 0 and row['executionTimeSeconds'] > 0.05 * totalRuntimeSeconds:
            ...
            ax.text(textXPosMinutes, yPos, timeLabel, ha='center', va='center', color=textColor, fontsize=defaultFontSize, path_effects=[mpe.Stroke(linewidth=2, foreground=backgroundColor), mpe.Normal()])

def draw_group_annotations(ax, timeDf, functionMap, numFunctions, groupLabelColor, groupBoxEdgeColor, groupBoxPaddingY, groupLabelYOffset, groupLabelMaxCharsPerLine, estimatedLineHeightForGroupLabel, groupLabelFontSize):
    ...
    for (groupIdx, (groupName, functionsInGroupMap)) in enumerate(functionMap.items()):
        ...
        if rectWidthMinutes < 0:
            rectWidthMinutes = 0
        rect = patches.Rectangle((rectXStartMinutes, rectYStart), rectWidthMinutes, rectHeight, linewidth=0.75, edgecolor=groupBoxEdgeColor, facecolor='none', zorder=0.5)
        ax.add_patch(rect)
        ...
        ax.text(labelXPos, labelYPos, finalLabelTextForGroup, ha='left', va='center', color=groupLabelColor, fontsize=groupLabelFontSize, weight='bold', clip_on=False, bbox=bboxProps)
...
class HandlerStyledIdBox(HandlerBase):
    ...
    def __init__(self, textColorName, boxFacecolorName, boxEdgecolorName, fontWeight='bold', fontFamily='sans-serif', boxStyleStr='square,pad=0.3', fontSizeFactor=0.9, boxLinewidth=1):
        super().__init__()
    ...
    def create_artists(self, legend, origHandle, xdescent, ydescent, width, height, fontsize, trans):
        groupIdStr = str(origHandle)
        textArtist = mtext.Text(width / 2.0, height / 2.0, groupIdStr, color=self.textColorName, fontsize=fontsize * self.fontSizeFactor, ha='center', va='center', fontweight=self.fontWeight, fontfamily=self.fontFamily, transform=trans)
        bboxProps = dict(boxstyle=self.boxStyleStr, facecolor=self.boxFacecolorName, edgecolor=self.boxEdgecolorName, linewidth=self.boxLinewidth)
        textArtist.set_bbox(bboxProps)
        return [textArtist]

def finalize_plot_styling(fig, ax, timeDf, displayNameMap, yMaxForPlot, textColor, elementColor, backgroundColor, defaultFontSize, groupLegendDetails, groupLabelColor, groupBoxEdgeColor):
    ...
    if groupLegendDetails:
        legendHandles = []
        legendLabels = []
        styledIdKeyHandler = HandlerStyledIdBox(textColorName=groupLabelColor, boxFacecolorName='black', boxEdgecolorName=groupLabelColor, fontSizeFactor=0.85, boxStyleStr='square,pad=0.3', boxLinewidth=1)
        handlerMapForLegend = {}
        ...
        ax.legend(legendHandles, legendLabels, fontsize=defaultFontSize, title_fontsize=defaultFontSize * 0.9, loc='lower left', bbox_to_anchor=(0.0, 0.0), facecolor=backgroundColor, edgecolor='lime', labelcolor=textColor, handler_map=handlerMapForLegend)
...
def generate_gantt_chart(config):
    ...
    for (funcNameKey, detailsDict) in timeInfo.items():
        dataForDf.append({'functionName': funcNameKey, 'executionTimeSeconds': detailsDict['executionTime']})
    ...
    (yMaxFromGroupLabels, groupLegendDetails) = draw_group_annotations(ax, timeDf, functionMap, numFunctions, groupLabelColor, groupBoxEdgeColor, groupBoxPaddingY, groupLabelYOffset, groupLabelMaxCharsPerLine, estimatedLineHeightForGroupLabel, groupLabelFontSize)
    yMaxForPlot = max(yMaxForPlot, yMaxFromGroupLabels)
    finalize_plot_styling(fig, ax, timeDf, displayNameMap, yMaxForPlot, textColor, elementColor, backgroundColor, defaultFontSize, groupLegendDetails, groupLabelColor, groupBoxEdgeColor)
...
def make_charge_visualisation(config, outDir):
    ...
    view.setStyle({'stick': {'radius': 0.1, 'color': 'green'}, 'sphere': {'scale': 0.5, 'opacity': 0.75, 'colorscheme': {'prop': 'b', 'gradient': 'linear', 'colors': chargeGradientColors}}})
    view.setBackgroundColor('black')
    for (index, row) in chargesDf.iterrows():
        view.addLabel(row['ATOM_NAME'], {'fontSize': 10, 'fontColor': 'yellow', 'showBackground': True, 'backgroundOpacity': 1, 'backgroundColor': 'black', 'alignment': 'topCenter'}, {'serial': index + 1})
        view.addLabel(f"{round(row['Charge'], 2):.2f}", {'fontSize': 10, 'fontColor': 'yellow', 'showBackground': True, 'backgroundOpacity': 1, 'backgroundColor': 'black', 'alignment': 'bottomCenter'}, {'serial': index + 1})
    view.zoomTo()
...
def make_conformer_visualisations(config, outDir):
    ...
    for (idx, xyzFile) in enumerate(conformerXyzs):
        ...
        view.setStyle({'elem': 'C'}, {'stick': {'radius': 0.2, 'color': color}, 'sphere': {'scale': 0.3, 'color': color}})
        view.setStyle({'elem': 'C', 'invert': True}, {'stick': {'radius': 0.2}, 'sphere': {'scale': 0.3}})
        view.setBackgroundColor('black')
...
def make_highlighted_torsion_visualisations(config, outDir):
    ...
    for (torsionTag, dihedralData) in uniqueRotatableDihedrals.items():
        ...
        view.setStyle({'atom': dihedralData['ATOM_NAMES']}, {'sphere': {'scale': 0.3, 'color': '#00FF00'}, 'stick': {'radius': 0.2}})
        view.setBackgroundColor('black')
