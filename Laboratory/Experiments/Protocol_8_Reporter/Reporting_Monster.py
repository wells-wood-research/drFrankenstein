...
def process_fitting_results(config: dict) -> dict:
    ...
    fittingData = {'nShuffles': nShuffles, 'rotatableDihedrals': config['runtimeInfo']['madeByTwisting']['rotatableDihedrals'], 'finalParameters': config['runtimeInfo']['madeByStitching']['finalParameters'], 'maxCosineFunctions': config['parameterFittingInfo']['maxCosineFunctions']}
    fittingImages = {}
    for torsionTag in os.listdir(qmmmParameterFittingDir):
        ...
        fittingImages[torsionTag] = {'finalPng': relativePng, 'fittingGif': relativeGif}
...
def process_charges_results(config: dict) -> dict:
    ...
    chargesData = {'chargeFittingProtocol': config['chargeFittingInfo']['chargeFittingProtocol'], 'optMethod': config['chargeFittingInfo']['optMethod'], 'optSolvationMethod': config['chargeFittingInfo']['optSolvationMethod'], 'singlePointMethod': config['chargeFittingInfo']['singlePointMethod'], 'singlePointSolvationMethod': config['chargeFittingInfo']['singlePointSolvationMethod']}
...
def process_twist_results(config: dict) -> dict:
    ...
    twistData = {'scanMethod': config['torsionScanInfo']['scanMethod'], 'scanSolvationMethod': config['torsionScanInfo']['scanSolvationMethod'], 'singlePointMethod': config['torsionScanInfo']['singlePointMethod'], 'singlePointSolvationMethod': config['torsionScanInfo']['singlePointSolvationMethod'], 'nRotatableBonds': config['runtimeInfo']['madeByTwisting']['nRotatableBonds'], 'rotatableDihedrals': deepcopy(config['runtimeInfo']['madeByTwisting']['rotatableDihedrals'])}
...
def process_wriggle_results(config: dict) -> dict:
    ...
    wriggleData = {'conformerHtmls': conformerHtmls, 'conformerEnergies': config['runtimeInfo']['madeByConformers']['conformerEnergies'], 'nConformersGenerated': config['runtimeInfo']['madeByConformers']['nConformersGenerated']}
    return wriggleData