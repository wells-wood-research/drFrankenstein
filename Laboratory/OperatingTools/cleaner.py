...
def clean_wriggle(config: dict) -> None:
    ...
    if cleanUpLevel == 0:
        ...
        moleculeName = config['moleculeInfo']['moleculeName']
        filesToKeep = [file for file in os.listdir(conformerDir) if file.startswith(f'{moleculeName}_conformer_') and file.endswith('.xyz')]
...
def clean_up_scan_dir(scanDir: DirectoryPath, config: dict) -> None:
    ...
    if cleanUpLevel == 0:
        ...
        keepFiles.extend(['orca_scan.relaxscanact.dat', 'orca_scan.inp', 'ORCA_FINISHED_NORMALLY', 'ORCA_CRASHED'])
...
def clean_up_solvator_dir(solvatorDir: DirectoryPath, config: dict) -> None:
    ...
    if cleanUpLevel == 0:
        ...
        keepFiles = [file for file in os.listdir(solvatorDir) if file.endswith('.inp') or file.endswith('.out') or file.endswith('.xyz')]
    elif cleanUpLevel in [2, 3]:
...
def clean_up_stitching(config: dict) -> None:
    ...
    if cleanUpLevel in [1, 2, 3]:
        ...
        paramFiles = [p.join(moleculeParameterDir, file) for file in os.listdir(moleculeParameterDir) if file.endswith('.prm') or file.endswith('.frcmod')]
