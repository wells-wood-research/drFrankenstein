...
def get_MM_total_energies(config, torsionTag, debug=True):
    ...
    ## sort out data
    mergedEnergyDf = shared_utils.merge_energy_dfs(singlePointEnergyDfs)
...
def get_singlepoint_energies_for_torsion_scan(scanIndex, scanDir, torsionTotalDir, cappedPdb, chargesDf, moleculeFrcmod, debug=False):
    ...
    singlePointEnergies = shared_utils.run_mm_singlepoints(trajPdbs, prmtop, inpcrd)


    singlePointEnergyDf = pd.DataFrame(singlePointEnergies, columns=["TrajIndex", "Energy"])
    singlePointEnergyDf["Angle"] = shared_utils.get_scan_angles_from_orca_inp(scanDir)
    singlePointEnergyDf["Angle"] = singlePointEnergyDf["Angle"].apply(shared_utils.rescale_angles_0_360)
...
def convert_traj_xyz_to_pdb(trajXyzs, cappedPdb, fittingRoundDir):
    ...
    for trajXyz in trajXyzs:
        ...
        shared_utils.update_pdb_coords(cappedPdb, trajXyz, trajPdb)
...
def make_prmtop_inpcrd(trajXyz, fittingRoundDir, cappedPdb, chargesDf, moleculeFrcmod, debug=False):
    ...
    shared_utils.pdb2mol2(trajPdb, trajMol2, fittingRoundDir)

    chargedMol2 = p.join(fittingRoundDir, f"orca_{trajIndex}_charged.mol2")
    shared_utils.edit_mol2_partial_charges(trajMol2, chargesDf, chargedMol2)

    renamedMol2 = p.join(fittingRoundDir, f"orca_{trajIndex}_charged_renamed.mol2")
    shared_utils.edit_mo2_atom_types(chargedMol2, renamedMol2)
    prmtop, inpcrd = shared_utils.make_prmtop_and_inpcrd(renamedMol2, moleculeFrcmod, fittingRoundDir, trajIndex)
